/*
 * WizardPage — Workshop Blueprint Design
 * "Before You Call" — Electrical Troubleshooting Wizard
 ...
 * Includes auto-save diagnosis on terminal nodes
 */
import { useState, useCallback, useRef, useEffect } from "react";
import { Link } from "wouter";
import { motion, AnimatePresence } from "framer-motion";
import { WIZARD_TREE, type WizardNode, type NodeType } from "@/lib/wizardData";
...
function ProgressDots({
  ...
  const types: NodeType[] = history.map((id) => WIZARD_TREE[id]?.type || ("question" as NodeType));
  ...
  return (
    <div className="bg-gradient-to-r from-red-950/80 to-red-900/60 border border-red-800/40 rounded-xl px-4 py-3 flex items-center gap-3 mb-5">
      ...
      <p className="text-red-300 text-sm font-serif leading-snug">
        ...
        smokin', it's broken. Shut it off. Use your senses. Call a pro.
      </p>
    </div>
  );
}
    ...
    return (
      <div className="bg-card rounded-2xl p-6 md:p-7 shadow-lg">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => onSelect(node.yesResult!, "Yes — I see this")}
...
export default function WizardPage() {
  ...
  const saveDiagnosis = useCallback((terminalId: string) => {
    const terminalNode = WIZARD_TREE[terminalId];
    if (!terminalNode) return;

    const steps: DiagnosisStep[] = [...history, terminalId].map((id) => {
      const n = WIZARD_TREE[id];
      return {
        nodeId: id,
        title: n?.title || id,
        icon: n?.icon || "❓",
    ...
    const diagnosis: SavedDiagnosis = {
      ...
      wizardType: "electrical",
      wizardLabel: "Electrical",
      timestamp: Date.now(),
      steps,
      outcome: {
        type: terminalNode.type as "resolved" | "call-tech" | "stop",
        title: terminalNode.title,
        message: terminalNode.message || terminalNode.title,
        script: terminalNode.script,
        tip: terminalNode.tip,
      },
    };
  ...
  // Auto-save when reaching a terminal node
  useEffect(() => {
    if (isTerminal && !saved) {
      saveDiagnosis(currentId);
    }
  ...
  const goTo = useCallback(
    (id: string, optionLabel?: string) => {
      if (optionLabel) {
        setSelectedOptions((prev) => ({ ...prev, [currentId]: optionLabel }));
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-navy via-[#0F2847] to-[#162F54]">
        ...
        <div className="max-w-xl mx-auto px-5 py-3 flex items-center justify-between">
          <div>
            ...
            <p className="font-mono text-[10px] text-safety-orange tracking-widest uppercase ml-[30px]">
              Before You Call &middot; Electrical
            </p>
          </div>
      ...
      <div className="max-w-xl mx-auto px-5 pt-4">
        <ProgressDots history={history} currentType={node?.type || "question"} />
      ...
      <div className="max-w-xl mx-auto px-5 py-6 text-center">
        ...
        >
          ...
          Back to Before You Call
        </Link>
        <p className="text-white/15 text-[11px] font-mono leading-relaxed mt-4">
          Before You Call &middot; Educational guidance only.
          <br />
          Always consult a licensed professional for repairs. Never work inside
          your breaker panel.
        </p>
      </div>
    </div>
