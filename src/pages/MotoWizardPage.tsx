/*
 * MotoWizardPage — Workshop Blueprint Design
 * "Before You Call" — Motorcycle Troubleshooting Wizard: Motorcycle Won't Start
...
import { MOTO_WIZARD_TREE, type MotoWizardNode, type MotoNodeType } from "@/lib/motoWizardData";
...
function ProgressDots({
  ...
  history: string[];
  currentType: MotoNodeType;
}) {
  const types: MotoNodeType[] = history.map(
    (id) => MOTO_WIZARD_TREE[id]?.type || ("question" as MotoNodeType)
  );
  ...
  return (
    <div className="bg-gradient-to-r from-red-950/80 to-red-900/60 border border-red-800/40 rounded-xl px-4 py-3 flex items-center gap-3 mb-5">
      ...
      <p className="text-red-300 text-sm font-serif leading-snug">
        ...
        smokin', it's broken. Never work on a hot engine or near fuel leaks.
      </p>
    </div>
  );
}
...
export default function MotoWizardPage() {
  ...
  const saveDiagnosis = useCallback((terminalId: string) => {
    const terminalNode = MOTO_WIZARD_TREE[terminalId];
    if (!terminalNode) return;
    const steps: DiagnosisStep[] = [...history, terminalId].map((id) => {
      const n = MOTO_WIZARD_TREE[id];
    ...
    const diagnosis: SavedDiagnosis = {
      id: generateDiagnosisId(), wizardType: "motorcycle", wizardLabel: "Motorcycle Won't Start",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-navy via-[#0F2847] to-[#162F54]">
        ...
        <div className="max-w-xl mx-auto px-5 py-3 flex items-center justify-between">
          <div>
            ...
            <p className="font-mono text-[10px] text-safety-orange tracking-widest uppercase ml-[30px]">
              Before You Call &middot; Motorcycle
            </p>
          </div>
      ...
      <div className="max-w-xl mx-auto px-5 pt-4">
        <ProgressDots history={history} currentType={node?.type || ("question" as MotoNodeType)} />
      ...
      <div className="max-w-xl mx-auto px-5 py-6 text-center">
        ...
        <p className="text-white/15 text-[11px] font-mono leading-relaxed">
          ...
          <br />
          Always consult a certified motorcycle technician for repairs.
          <br />
          Never work on a hot engine or near fuel leaks.
        </p>
      </div>
    </div>
  );
