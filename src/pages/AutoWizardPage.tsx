/*
 * AutoWizardPage — Workshop Blueprint Design
 * "Before You Call" — Automotive Troubleshooting Wizard: Car Won't Start
...
import { AUTO_WIZARD_TREE, type AutoWizardNode, type AutoNodeType } from "@/lib/autoWizardData";
...
function ProgressDots({
  ...
  history: string[];
  currentType: AutoNodeType;
}) {
  const types: AutoNodeType[] = history.map(
    (id) => AUTO_WIZARD_TREE[id]?.type || ("question" as AutoNodeType)
  );
  ...
  return (
    <div className="bg-gradient-to-r from-red-950/80 to-red-900/60 border border-red-800/40 rounded-xl px-4 py-3 flex items-center gap-3 mb-5">
      ...
      <p className="text-red-300 text-sm font-serif leading-snug">
        ...
        smokin', it's broken. Don't keep cranking. Use your senses. Call a pro.
      </p>
    </div>
  );
}
...
export default function AutoWizardPage() {
  ...
  const saveDiagnosis = useCallback((terminalId: string) => {
    const terminalNode = AUTO_WIZARD_TREE[terminalId];
    if (!terminalNode) return;
    const steps: DiagnosisStep[] = [...history, terminalId].map((id) => {
      const n = AUTO_WIZARD_TREE[id];
    ...
    const diagnosis: SavedDiagnosis = {
      id: generateDiagnosisId(), wizardType: "automotive", wizardLabel: "Car Won't Start",
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-navy via-[#0F2847] to-[#162F54]">
        ...
        <div className="max-w-xl mx-auto px-5 py-3 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2.5">
              <Car className="w-5 h-5 text-safety-orange" />
            ...
            <p className="font-mono text-[10px] text-safety-orange tracking-widest uppercase ml-[30px]">
              Before You Call &middot; Automotive
            </p>
          </div>
      ...
      <div className="max-w-xl mx-auto px-5 pt-4">
        <ProgressDots history={history} currentType={node?.type || ("question" as AutoNodeType)} />
      ...
      <div className="max-w-xl mx-auto px-5 py-6 text-center">
        ...
        <p className="text-white/15 text-[11px] font-mono leading-relaxed">
          ...
          <br />
          Always consult a licensed mechanic for repairs. Never work under an
          unsupported vehicle.
        </p>
      </div>
    </div>
